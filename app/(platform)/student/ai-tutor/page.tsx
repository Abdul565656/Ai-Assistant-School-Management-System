// app/(platform)/student/ai-tutor/page.tsx
"use client";
...
import { ScrollArea, ScrollBar } from '@/components/ui/scroll-area'; // Ensure ScrollBar is imported if needed
import { getAITutorResponse } from '@/lib/actions/ai.actions'; // Server Action
import { toast } from 'sonner';
import { useSession } from 'next-auth/react'; // To get student's info if needed
...
// Type for messages sent to the server action (matches ChatContent in ai.actions.ts)
...
export default function AITutorPage() {
  ...
  const scrollViewportRef = useRef<HTMLDivElement>(null); // Ref for the viewport of ScrollArea
  ...
  const handleSendMessage = useCallback(async (e?: React.FormEvent<HTMLFormElement>) => {
    ...
    // This includes all previous messages + the current user message
    ...
    try {
      // The server action expects the current message separately and then the history (which now includes it)
      ...
      // Optional: Remove the optimistically added user message or mark it as failed
      setMessages(prev => prev.filter(msg => msg.id !== userMessage.id)); // Example: remove user message on error
      // Or add an error message from AI
      // const errorMessage: DisplayMessage = { /* ... sender: 'ai', text: 'Error...' */ };
      // setMessages(prev => [...prev, errorMessage]);
    } finally {
      setIsLoading(false);
    }
  }, [inputValue, isLoading, messages]); // Include all dependencies
    ...
    <Card className="w-full h-[calc(100vh-10rem)] md:h-[calc(100vh-12rem)] flex flex-col shadow-xl border">
      ...
      <CardContent className="flex-grow p-0 overflow-hidden">
        <ScrollArea className="h-full" type="auto">
          {/* The actual scrollable viewport needs the ref */}
          <div ref={scrollViewportRef} className="h-full p-4 md:p-6 space-y-6">
            {messages.length === 0 && !isLoading && (
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                ...
                <p>I'm EduBot, your AI Tutor. How can I help you learn today?</p>
                <p className="text-xs mt-2">(e.g., "Explain photosynthesis like I'm 5")</p>
              </div>
            )}
            {messages.map((msg) => (
              <div
              ...
              >
                ...
                >
                  {/* Simple text rendering, consider react-markdown for complex markdown */}
                  {msg.text.split('\n').map((line, index, arr) => (
            ...
            {isLoading && messages.length > 0 && ( // Show loader only if there are previous messages
