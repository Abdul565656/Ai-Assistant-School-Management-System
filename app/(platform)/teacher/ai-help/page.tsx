...
export default function AITeachingHelpPage() {
  ...
  const teacherInitials = getInitials(session?.user?.name, 'T');
  const teacherAvatar = session?.user?.image;
    ...
    <Card className="w-full h-[calc(100vh-10rem)] md:h-[calc(100vh-12rem)] flex flex-col shadow-xl border">
      <CardHeader className="border-b px-4 py-3 sm:px-6 sm:py-4">
        <CardTitle className="flex items-center gap-2 text-lg sm:text-xl">
          <Brain className="h-6 w-6 text-primary" /> AI Teaching Assistant - EduPro
        </CardTitle>
      </CardHeader>
          ...
          <div ref={scrollViewportRef} className="h-full p-4 md:p-6 space-y-6">
            ...
            {messages.map((msg) => (
              ...
              >
                ...
                {msg.sender === 'user' && (
                  <Avatar className="h-8 w-8 border">
                    {teacherAvatar ? <AvatarImage src={teacherAvatar} alt="Teacher"/> : null}
            ...
            {isLoading && messages.length > 0 && (
              <div className="flex items-end gap-2.5 justify-start">
                <Avatar className="h-8 w-8 border"><AvatarFallback className="bg-primary text-primary-foreground text-xs"><Brain size={16} /></AvatarFallback></Avatar>
      ...
      <CardFooter className="border-t p-3 sm:p-4">
        <form onSubmit={handleSendMessage} className="flex w-full items-center space-x-2">
          <Input
            type="text"
            placeholder="Ask EduPro for teaching help..."
            value={inputValue}
